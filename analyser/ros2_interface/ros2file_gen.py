...
class BagCreator:
    def __init__(
            self,
            output_directory_address,
            ros_file_name):
        
        self.writer = Writer(output_directory_address + '/rosbag2_' + ros_file_name)
        self.writer.open()
    ...
    def write_messages(
        ...
        try:
            ...
            connection = self.writer.add_connection(topic, msgtype)
    ...
    def write_netstat_messages(
        ...
        self.write_messages(topic, netstat_messages)
    ...
    def write_obu_messages(
        ...
        self.write_messages(topic, dmn_messages)
    ...
    def write_rsu_messages(
        ...
        self.write_messages(topic, rsu_messages)


    def create_and_add_messages(
            self,
            csv_dict_input: dict) -> None:
        """
        ...
        for topic in list(csv_dict_input.keys()):
            if "OBU" in topic:
                if Conf.network_status_topic_name_syntax in topic:
                    self.write_netstat_messages(
                        topic,
                        csv_dict_input[topic])
                else:
                    self.write_obu_messages(
                        topic,
                        csv_dict_input[topic][0],
                        csv_dict_input[topic][1])
            else:
                self.write_rsu_messages(
                    topic,
                    csv_dict_input[topic][0],
