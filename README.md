...
In the Infrastructure tab, you will need to configure for the present RSU/OBUs plus Clouds and Traffic Signals for visualisation purposes. Hovering the mouse over the line edits will display a useful tooltip text to give you hints and examples to fill in the configurations. The rest of the process is pretty straightforward and intuitive. We've also provided you with a sample configuration text file for the entities under `sample_data`. The `sample_data` directory dates back to a series of experiments taken between 19 to 22 of February 2024. The data belongs to 19/02/2024.
