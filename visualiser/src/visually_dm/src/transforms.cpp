    ...
    , const double transform_matrix[4][4])
{
    ...
    multiplyMatrices(transformed_point, transform_matrix, original_point);
    for(int i{ 0 }; i < 3; ++i)
        result[i] = transformed_point[i][0];
}

void pose2Array(const geometry_msgs::msg::Pose& pose, double position[3], double orientation[3])
{
...
void toMgrs(double lon, double lat, double& x, double& y)
{
    static int zone;
    static bool northp;
